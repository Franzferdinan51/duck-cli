...
### Architecture — Hub & Spoke with Meta Agent Coordination
...
│    └─────────────┘    └─────────────┘    └─────────────┘                   │
│                                                                              │
│    Each meta agent connects to ALL layers:                                   │
│    • Access Layer (CLI, Telegram, Web) - via Bridge                          │
│    • Agent Core (orchestration, tools) - via Orchestrator                    │
│    • Mesh Bus (communication) - all agents connected                         │
│    • Bridge Layer (external systems) - via Bridge Agent                      │
│    • Internal Agents (council, subconscious) - via Mesh                      │
└─────────────────────────────────────────────────────────────────────────────┘
           │                  │                         │
           ▼                  ▼                         ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                         AGENT MESH BUS (Port 4000)                          │
│                    (All agents auto-register here - REQUIRED)                │
│                                                                              │
│   ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │
│   │ AI Council   │  │ Sub-Conscious│  │   KAIROS     │  │ Skill Creator│   │
│   │ (Deliberate) │  │ (Whispers)   │  │ (Heartbeat)  │  │ (Auto-skill) │   │
│   └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │
│                                                                              │
│   Meta agents coordinate via mesh:                                           │
│   • Orchestrator → routes tasks to appropriate agents                        │
│   • Bridge → connects mesh to external systems                               │
│   • Subconscious → monitors mesh for patterns                                │
│   • Mesh Coordinator → manages mesh topology                                 │
...
│        └─────────────┴──────────────┴────────────────┘                      │
│                         │                                                    │
│              Meta agents bridge internal ↔ external:                         │
│              • Bridge Agent coordinates all external calls                   │
│              • Security Agent validates external requests                    │
│              • Monitor Agent tracks bridge health                            │
└─────────────────────────────────────────────────────────────────────────────┘
           │                  │                         │
           ▼                  ▼                         ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
**Meta Agent Hub Pattern:**

Unlike a stack, meta agents form a **coordination hub** that connects to ALL layers:

| Meta Agent | Model | Connects To | Role |
|------------|-------|-------------|------|
| **Orchestrator** | qwen3.5-2b-claude-4.6-opus-reasoning-distilled | Access, Core, Mesh, Bridge | Routes tasks, selects models |
| **Bridge** | gemma-4-e2b-it | Mesh, Bridge, External | Connects internal ↔ external |
| **Subconscious** | qwen3.5-0.8b | Mesh, Core, Memory | Watches patterns, generates whispers |
| **Mesh Coordinator** | gemma-4-e2b-it | Mesh, All Agents | Manages mesh topology, federation |
| **AI Council Liaison** | qwen3.5-2b-claude-4.6-opus-reasoning-distilled | Mesh, Council, Bridge | Coordinates council deliberation |
| **Monitor** | qwen3.5-0.8b | Mesh, Bridge, Core | Tracks health, generates alerts |
| **Memory** | qwen3.5-0.8b | Mesh, Core, All Agents | Manages persistence, recall |
| **Security** | qwen3.5-2b-claude-4.6-opus-reasoning-distilled | Mesh, Bridge, External | Validates, audits, protects |
| **Scheduler** | qwen3.5-0.8b | Mesh, Core, All Agents | Plans tasks, manages cron |
...
| **Meta Agents** | 9 internal agents with tools & time context | System coordination hub |
| **Mesh Bus** | **Required** internal communication layer (port 4000) | All internal agents auto-register |
...
**1. Mesh is Required - Auto-Registration**
All internal agents MUST auto-connect to the mesh bus (port 4000) on startup:
...
**2. Meta Agents are a Hub**
Meta agents don't just sit in a layer — they connect to ALL layers:
- Horizontal: Coordinate with each other via mesh
- Vertical: Connect Access ↔ Core ↔ Bridge ↔ External
- Each has specific coordination responsibilities
...
## 🎭 Meta Agents — Internal Coordination
...
./duck meta task <type> <task>  # Assign task to agent type
```
...
│   ├── agent/                  # Core agent + Android tools + Meta Agents
